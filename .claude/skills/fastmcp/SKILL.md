...
### Primary Method: Direct Page Access (Recommended)

Refer to ./llms.txt to as it contains the URLs to the documentation of that topic and you can fetch specific pages directly for that topic:

```
WebFetch(
    url="<url of markdown documentation fetched from llms.txt>"
)
```

### Alternative: Use WebSearch 

An alternate but still a reliable way to find FastMCP documentation is using WebSearch with site-specific queries:
...
1. **User asks about OpenAI integration:**
   ...
   Recommended way:
   ```
   Fetch URL to Markdown from llms.txt → Websearch(URL) → Fetch the markdown documentation for details 
   ```
