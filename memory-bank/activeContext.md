...
## Current Phase — v1→v2 MIGRATION IN PROGRESS (2026-06-26): E0 + E1 (P0) + E2.S1 (P1) + E2.S2 (P2) + E3.S1 (P3) + E3.S2 (P4) landed
The §15 migration is executing. Plans: [../claudedocs/workflow_v1_to_v2_migration.md] (phases P0–P7) + [../claudedocs/spawn_v1_to_v2_migration.md] (epics E0–E5). **Done: E1 (P0 Tier-0) + E0 (live-path hardening), E2.S1 (P1 App + one-node Workflow skeleton), E2.S2 (P2 research-loop port), E3.S1 (P3 real RequestInput CP2/CP3 + reject path), E3.S2 (P4 flip session ownership to ADK).**
...
- **E3.S1 / P3 ✅ (2026-06-25)** — real `RequestInput` CP2/CP3 + reject path (subagent-driven; fresh subagent per task + `cavecrew-reviewer` between; ADK 2.3.0 re-verified via `adk-docs` MCP). Order: T0 ‖ T2 → T1 → T3 → T4 (tracking).
  - **T0 (`21907e0`)** — Writer node wired; DI seam `writer_node=None` on `build_research_workflow`; renders markdown draft via `render_report()` after the loop; draft carried to CP2. Terminal output stays `ClaimLedger` (parity-neutral, additive).
  - **T2 (`dd72272`)** — NEW pure `app/cp3_adapter.py`: `apply_cp3_verbs(urls, reply) -> (filtered_urls, needs_supplemental)` ports v1 verbs exactly (`+ <url>` add, `- <n>` exclude, `r <url>` redirect, `d` done); `_user_url -> ScoredURL(score=1.0, strategy=CRAWL, source="user")`; imports config+schemas only. CP3 is now a real deep-only `RequestInput` node (`_make_cp3_checkpoint(interrupt_id=f"cp3_{subtopic.id}_{iteration}")`); `needs_supplemental` → acquirer re-entry before extract. +11 unit tests `test_cp3_adapter.py`. `cp3_hook` seam removed.
...
- **E3.S2 / P4 ✅ (2026-06-26)** — session ownership flipped to ADK; `SessionStore` demoted to exporter; `stage.json` retired as truth for v2 (subagent-driven; `cavecrew-reviewer` between each task; ADK 2.3.0 re-verified via `adk-docs` MCP).
  - **T1 (`2217d73`)** — `DatabaseSessionService` + `build_runner()` factory + `yield Event(state={…})` state-delta calls in `research` @node (`v2_plan`/`v2_ledger`/`v2_draft`/loop markers). Plan A confirmed: `yield Event(state=…)` from inside a `@node` generator IS durable. `research` converted from coroutine to async generator.
...
- **Suite: 261 orchestrator (+8 E3.S2) + 43 MCP green, ruff clean. COMMITTED** to `dev` — E3.S2 `2217d73`/`19bf987`/`27e7c6d`/`62e8ba5`/`e283f35`/`b03b339`; E3.S1 `21907e0`/`dd72272`/`57d1755`/`522f0c4`; E2.S2 `8547088`/`8f0ace0`/`17d7be4`/`0dd0bfe`; E2.S1 `21330ef`/`306f8d7`/`95cc51b`/`850b75b`. Not pushed.
- **Next = E4 / P5** (expose pipeline as local-first A2A via `to_a2a`/`adk api_server --a2a` + CLI). All 3 HARD GATES green (E2.S1 resume, E2.S2 parity, E3.S2 byte-comparable export). E4–E5 still pending.
