...
Status: **DESIGN v2 (ratified 2026-06-23)** — commits the project to **ADK 2.x**, **ADK-owned sessions**, **dynamic-workflow** orchestration, and a **single local-first A2A boundary**. Supersedes the v1 hand-rolled-orchestrator design that Stories 1–5 implemented; §15 is the migration path. Section numbers ≤ §12 are preserved so existing code `§`-references stay valid; §13–§15 are new. Input review: [../claudedocs/adk_alignment_review.md]. Requirements: [requirements.md].
...
| Decision | Choice | Why |
|----------|--------|-----|
| Framework + major | **Google ADK, pinned `>=2.3,<3`** | Already running ADK 2.3.0 (uv.lock); 2.0 GA'd 2026-05-19. The loose `>=0.3.0` floor spanned the 1.x→2.0 break and must be pinned (review §0.2). |
| Orchestration substrate | **ADK 2.0 dynamic workflows** (`@node` / `Workflow` / `ctx.run_node`) | Near-1:1 fit for the hand-rolled stage machine + research loop; gives automatic checkpoint-resume and native ADK sessions; retires `orchestrator.py` / `stage_machine.py` / `_run_sync` (review §2). |
| Session & resume ownership | **ADK session + event store** (`ResumabilityConfig(is_resumable=True)`) | ADK persists step/event state; resume re-runs only unfinished nodes. `data/sessions/{id}/*.json` demoted to a **write-through export**, not the source of truth (§5). |
| Inter-agent topology | **Local sub-agents / workflow nodes** on one hot model | The six specialists share one resident model and run sequentially (16 GB RAM). ADK's A2A guidance is explicit: in-process, shared-model, performance-coupled agents should be *local sub-agents*, **not** A2A peers — A2A's network+serialization overhead would buy nothing here. |
| A2A boundary | **One A2A server at the pipeline edge, local-first** | The whole Deep-Research pipeline is exposed as a single `A2AServer` (auto agent card), consumable over **localhost** by a local client or another ADK agent via `RemoteA2aAgent`. Real A2A contract at the edge; no internal A2A overhead (§13). |
| Runtime / front-end | **`adk api_server` (local service)** + thin CLI | A2A exposure, `RequestInput` HITL over API/SSE, and resume-by-`invocation_id` all work natively. Single box, local Ollama — no cloud. |
| Tool transport | **MCP (unchanged)** | crawl4ai stays a stdio `MCPToolset`. MCP is the right boundary for tools; A2A is the right boundary for the agent. |

**Binding constraint (unchanged):** 16 GB **RAM** (not VRAM) is the limit ⇒ **sequential, single-hot-model** execution regardless of how many *logical* nodes exist. This is why the six are local nodes, not A2A microservices.
...
- **RAM discipline**: crawl4ai/Playwright is the RAM hog. Never run heavy crawl concurrently with large-batch inference — structural, because nodes run sequentially.
- **One model, many roles**: nodes differ by system prompt + tool access, not model. Reached via ADK `LiteLlm` → local Ollama with the `ollama_chat/` provider prefix.
- **Connectivity (review §0.1):** set the **`OLLAMA_API_BASE` env var** (not only the `api_base` param) at startup — LiteLLM routes non-generation calls through the env var. Without it, a non-default `OLLAMA_BASE_URL` silently hits localhost.

---

## 2. Agent Topology: Local Nodes Under One A2A Boundary

**Pattern: a dynamic-workflow root composes single-responsibility specialist nodes; the whole pipeline is the one A2A-exposed agent.** Specialists are local nodes (in-process, one hot model), never A2A peers, never calling each other directly — data flows through the workflow.
...
**Least-privilege tool access (unchanged):** only **Acquirer** (read: `discover_urls`, `score_and_triage_urls`) and **Extractor** (write: `crawl_*`) and **Verifier** (read: `search_chunks`) hold MCP tools; Clarifier/Planner/Writer are reasoning-only. The `MCPToolset` for each is built **once per run and reused** across loop passes, then `close()`d at run end (review §0.3) — no per-pass subprocess churn.

---

## 3. Agent Roles & Contracts

Each specialist: single responsibility, typed input artifact → typed output artifact. Schemas (`orchestrator/app/schemas.py`, Pydantic v2) are the **inter-node contracts** and are unchanged by the v2 design — only the *call mechanism* changes (a node is invoked via `await ctx.run_node(agent, input)`, which returns the output directly; no throwaway `Runner`/session extraction).

### 3.1 Root workflow (`@node` dynamic workflow, replaces the v1 orchestrator agent)
- Owns: control flow (stage order + research loop), stop-rule, checkpoint placement.
- Does **not** own session/resume persistence anymore — ADK does (§5).
...
- `needs_input` → surfaced to the user **before run start only** (a pre-flight `RequestInput`, §6).
...
- `target_evidence` per subtopic is a deterministic post-step scaled by `depth` (config `TARGET_EVIDENCE_*`) → drives stop-rule.
- → **Checkpoint 1**.

### 3.4 Acquirer (FR3.1, FR3.4, FR3.5)
- In: subtopic question(s) + source_classes.
- Tools: `discover_urls` (SerpAPI+DDG+arXiv), `score_and_triage_urls`; Semantic Scholar citation-graph API (utility in `app/citation.py`, **not** an MCP tool).
- **Citation BFS** (`source_class == academic` AND `plan.depth == "deep"` only): 2-hop BFS over the S2 citation graph, LLM relevance-gated per candidate; enriches `ScoredURL` with `publication_date` + `citation_refs` (§11).
...
- **Publication-date passthrough**: copies `ScoredURL.publication_date` into chunk metadata (no inference). Verifier reads it for temporal-drift.
- Out: page IDs; content + provenance + date live in the stores (handoff is IDs, not text).

### 3.6 Verifier (FR5, FR5.5, FR5.6) — the trust core
- In: subtopic; retrieves its own chunks via `search_chunks` (includes chunk `publication_date`).
- **Model owns content** (which chunks support/contradict; a 0–1 methodological-explicitness signal). **Deterministic Python owns policy** — re-applied to every parsed ledger:
...
- Out: structured markdown — exec summary → section per subtopic → sources → contradictions appendix (factual/methodological vs. a separate **Temporal Drift** sub-section). Every kept claim cites source URL(s).
...
The control flow is a single ADK **dynamic workflow** (`@node`), not a hand-rolled driver. Nodes are invoked with `await ctx.run_node(...)`, whose return value *is* the node output — eliminating the v1 `asyncio.run` bridge and the manual `output_key`-from-discarded-session extraction.
...
**Stage identity preserved.** The canonical stage names (`INTAKE → CLARIFY → PLAN → RESEARCH(ACQUIRE→[MID_ACQUIRE]→EXTRACT→VERIFY) → SYNTHESIZE → WRITE → DONE`) remain the project's shared vocabulary and the resume/observability anchors; in v2 they are positions in the workflow graph rather than entries in a hand-walked `Stage` enum.

**Stop-rule (resolves Open Q5, unchanged, pure):** subtopic done when corroborated-claim count ≥ `target_evidence`, OR a pass adds `< MIN_NEW_CLAIMS` (diminishing returns), OR per-subtopic iteration cap (`depth_budget`). These pure functions (`stop_rule`, `depth_budget`) port to v2 verbatim as inline calls / function-nodes.

**Resume semantics:** dynamic workflows checkpoint every node execution; on resume, completed nodes are skipped automatically. Parent nodes that call `ctx.run_node` must set `rerun_on_resume=True`. Tools may run **at least once** (possibly more) on resume — crawl/extract are idempotent (content keyed by URL in Chroma), so re-runs are safe; this is an explicit design invariant to preserve.

---

## 5. State & Persistence (resolves Open Q4 — v2: ADK-owned)

- **Source of truth = ADK session + event store.** Live stage, current subtopic, budgets, and per-node completion live in the ADK session; resume is driven by ADK via `App(resumability_config=ResumabilityConfig(is_resumable=True))` and the run's `invocation_id`.
- **crawl4ai SQLite + ChromaDB**: all crawled content + provenance + `publication_date`. Reused as-is; unaffected by the session-ownership change.
- **`data/sessions/{id}/*.json` = export, not truth.** `plan.json`, `claim_ledger.json`, `draft.md` are written **through** ADK state (a thin after-node/after-agent callback or plugin projects ADK state to disk) for human inspection, diffing, and report provenance. `stage.json` is retired as a resume mechanism — ADK owns resume. Final report still lands at `data/reports/{session_id}.md`.
- **Migration note:** the v1 `SessionStore` becomes an **exporter** (write-through projection), not the persistence/resume engine. See §15.

---

## 6. Checkpoint UX (resolves Open Q6 — v2: `RequestInput` HITL)

Human gates are ADK **`RequestInput`** nodes that pause the workflow and resume on user response — working over `adk api_server` (API/SSE), not bound to a local TTY.

- **CP1 (after Planner):** `yield RequestInput(message=..., payload=plan, response_schema=ResearchPlan)` → approve/edit/reject the plan.
- **CP2 (after Writer):** approve/edit/reject the draft.
- **CP3 (after Acquirer, `plan.depth == "deep"` only):** inspect/steer the candidate source list before extraction; add/exclude/redirect URLs. A `[+]`/`[r]` edit triggers a supplemental Acquirer pass (loop re-entry) before Extractor runs.
- **Pre-flight clarification** (Clarifier `needs_input`): a `RequestInput` *before run start only*.

**Local-first interaction:** the same workflow runs two ways — (a) a thin **CLI** that drives it in-process for terminal use, and (b) `adk api_server` for API/A2A/remote-HITL. `RequestInput`'s `response_schema` does **not** auto-coerce free-form human input, so CP3's URL-edit grammar needs a small normalizing adapter (or an agent node) between raw input and the structured payload.

---

## 7. crawl4ai MCP Extensions (resolves Open Q3 — unchanged)

Targeted additions to the existing server (extend, don't rebuild — most already shipped in Story 2):
1. **Content-similarity dedup** — collapse syndicated/mirrored pages (cosine ≥ 0.92) so the independence test is reliable. *(shipped: `tools/dedup.py`)*
2. **eTLD+1 field** — registrable domain on discovery output + chunk metadata. *(shipped: `app/domain.py`, `chunks.etld1`)*
3. **Seed-URL ingest** — force user URLs/files into the crawl set (FR3.2). *(shipped: `tools/seed.py`)*
...
6. **Semantic Scholar citation-graph client** — `orchestrator/app/citation.py` httpx utility (not an MCP tool), called by Acquirer for §11/§12. *(shipped)*

MCP stays the tool boundary (stdio `MCPToolset`); it is **not** migrated to A2A — tools belong on MCP.
...
- ✅ Stateless specialists; state centralized in the **ADK session** (not a bespoke store).
...
- ✅ **A2A used correctly**: one boundary at the pipeline edge (local-first), specialists kept local — not A2A microservices.
...
**Bounds:** max 2 hops; `CITATION_RELEVANCE_THRESHOLD` (default 0.7); per-subtopic BFS budget cap; bounded queue. **Resource:** LLM scoring uses the resident model (no swap); S2 calls are lightweight JSON. **Output:** enriched `ScoredURL[]` (`publication_date`, `citation_refs`) → normal Extractor pipeline.
...
`publication_date` flows S2 → `ScoredURL` → chunk metadata → Verifier. If a contradiction's date gap ≥ `TEMPORAL_DRIFT_THRESHOLD_MONTHS` (18) → `conflict_type="temporal_drift"`; older side `temporal_status="dated"`, newer `"current"`. Missing date → `"uncertain"`, never classified as drift. Report: separate "Temporal Drift" sub-section, informational framing ("field has evolved since [date]") — AC7.
...
- **`to_a2a(root_agent, port=8001)`** — wraps the root workflow into an A2A Starlette app served by `uvicorn`; auto-generates the agent card in-memory. Tightest control over what is exposed.
- **`adk api_server --a2a <folder>`** with an `agent-card.json` — also gives `adk web` for debug/test and can host multiple agents from one parent folder. Preferred when the api_server is already the runtime.

The agent card is published at the well-known path (`…/a2a/deep_research/.well-known/agent-card.json`), advertising the pipeline's skill ("local deep-research → vetted markdown report") and `text/plain` in / `text/markdown` (or `application/json`) out.

**Local-first consumption (the user's explicit requirement):** the exposed server binds to **localhost**. A local consumer reaches it without leaving the machine, two ways:
1. **Direct HTTP** — `POST /run_sse` (run + stream), `RequestInput` pauses for CP1/CP2/CP3, resume via `invocation_id`. The thin CLI can speak this.
2. **`RemoteA2aAgent`** — another local ADK agent consumes the pipeline as a sub-agent:
   ```python
...
**Future split seam (not built now):** because specialists communicate only through typed artifacts, any single heavy specialist (e.g. Acquirer + citation BFS) *could* later be lifted onto its own `A2AServer` on another box and consumed via `RemoteA2aAgent`, with no change to the contracts. Internal A2A is **not** adopted today (no concurrency gain on one hot model) — only the seam is preserved.
...
- **`App` object** is the top-level container: `App(name="deep_research", root_agent=root_agent, resumability_config=ResumabilityConfig(is_resumable=True), plugins=[...])`.
...
- **Cross-cutting concerns via callbacks/plugins** (review §1.2): structured logging/telemetry and the `data/sessions/*.json` export go in `BeforeAgentCallback`/`AfterAgentCallback` or an `App`-level Plugin — **not** in execution overrides (the 2.0 Workflow Runtime bypasses `_run_async_impl` overrides).
...
- **Observability:** ADK traces/metrics/logging + (optional) a tracing integration; the multi-pass research loop benefits from per-node spans.
- **Sequential by constraint, parallel-ready by substrate:** subtopic research stays sequential under the 16 GB limit, but dynamic workflows support `asyncio.gather(*[ctx.run_node(worker, st) ...])` with resume re-running only failed workers — a clean upgrade path if the hardware envelope ever grows.
