...
- FR1.2 Detect ambiguous scope (multiple plausible interpretations) + missing constraints (timeframe, region, version, audience, etc.).
- FR1.3 FR1.2 triggers → STOP, ask user follow-up. Else proceed autonomously.
- FR1.4 Clarification before run start only — mid-run pauses = approvals, not new clarifications.
...
- FR2.3 Produce research plan (sub-topics, source strategy, est. effort).
- FR2.4 **Checkpoint 1**: present plan to user for approval/edit before execution.
- FR2.5 **Checkpoint 3 (CP3)**: for `depth=deep` plans only — present source list + initial theme clusters after Acquirer completes, before Extractor crawls. User can add terms, exclude URLs, redirect subtopics. Skipped for shallow/normal plans.
...
- FR3.4 Extensible: new source connectors addable, no core rewrite.
- FR3.5 Citation network traversal: academic sources on `depth=deep` plans — Acquirer follows Semantic Scholar citation graph 2 hops deep. Relevance-gated (LLM scores candidate papers against subtopic question). BFS bounded by hop depth + per-subtopic budget cap.
...
- FR4.2 Delegate only where needed — minimize concurrent model load.
...
- FR5.2 Claim contradicted across sources → flag (retain both sides, don't silently pick).
- FR5.3 Every kept claim traceable to source URL(s).
- FR5.4 Contradictions surfaced in report, not discarded.
- FR5.5 Contradiction confidence scoring: each flagged contradiction carries `confidence_score` (three-tier rubric: source independence + recency delta + methodological explicitness) + `conflict_type` (factual | methodological | temporal_drift). Contradictions appendix sorted by score descending.
- FR5.6 Temporal claim dating: where source `publication_date` available, detect temporal drift (date delta ≥ `temporal_drift_threshold`, default 18 months). Classify as `temporal_drift`, not hard contradiction. Tag claims `temporal_status`. Surfaced in dedicated "Temporal Drift" sub-section of contradictions appendix.
...
## Acceptance Criteria
- AC1: Ambiguous one-liner ("jaguar performance") triggers follow-up; clear detailed query doesn't.
- AC2: Plan checkpoint (CP1) + draft checkpoint (CP2) both block until user acts.
...
- AC6: End-to-end run doesn't exceed 16 GB VRAM (no OOM).
- AC7: Planted temporal drift case (old source contradicted by newer source, date delta ≥ 18mo) classified as `temporal_drift`, appears in "Temporal Drift" sub-section, not "Factual Contradictions".

## Open Questions (design phase)
1. Model fit: single mid-size role-prompted model vs. small specialized models per agent? (16 GB RAM = bottleneck.)
